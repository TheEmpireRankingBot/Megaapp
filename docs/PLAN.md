...
| Access | **Single-owner username/password** | Reliable private access without email delivery; add household auth only when sharing becomes a real requirement. |
...
Scaffold Next.js + Tailwind + Supabase Postgres + Drizzle; single-owner password access; core tables (`items`, `entries`, `tags`, `reminders`); app shell with navigation; deploy pipeline to Vercel; JSON export endpoint.
